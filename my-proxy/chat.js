...
// Keeps OPENAI_API_KEY on the server and returns { text } to the Qualtrics chatbot.
//
// The chatbot sends: { prompt, system, history, model, temperature, max_tokens }
// This function returns:  { text: "..." }

export default async function handler(req, res) {
  // ---- CORS (Qualtrics is a different origin, so this is required) ----
  res.setHeader("Access-Control-Allow-Origin", "*");        // or lock to your Qualtrics domain
  ...
  try {
    ...
    // Build the messages array: system + prior turns + this prompt (no duplication).
    const messages = [];
